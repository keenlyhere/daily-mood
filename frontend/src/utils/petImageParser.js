...
export const petImageParser = (flavor, emotion) => {
...
export const petImages = () => {
    const cowImages = [];
    const cowFlavors = [];
    const cows = { cowImages, cowFlavors }

    for (let key in flavors) {
        cowImages.push(flavors[key].normal);
        cowFlavors.push(key);
    }
...
export const userPetImages = (userFlavors) => {
    const userCowImages = [];
    const userCowFlavors = [];
    const cows = { userCowImages, userCowFlavors };

    for (let key in flavors) {
        if (userFlavors.includes(key)) {
            userCowImages.push(flavors[key].normal);
            userCowFlavors.push(key);
        }
    }
